...
四问环环相扣，全部数值结果由真实附件数据计算得到，主要结果可由随附脚本
在附录 B 数据口径下复算。
...
相等的临界点 $L_{\text{ctx}}^{\text{crit}}=6/\eta=30000$。跨活跃集/无导数/全局进化/罚函数精修等\textbf{九类求解框架}在 9 组预算—成本
实例上交叉验证得到全局一致最优解。三档预算下最优分配
呈现清晰的结构性转移：预算低于约 $10^{18}$ 量级时最优 $Q=Q_0$（纯规模
扩张），越过阈值后质量投入激活，$10^{22}$ 量级时 $Q$ 饱和于 1，新增预算绝大部分
转向规模扩张；三种质量成本函数下的转移阈值分别为 $6.3\times10^{17}$、
$2.0\times10^{18}$、$3.2\times10^{18}$，且 $L_{\text{ctx}}$ 越长注意力份额
越高，在临界点处注意力份额恰好等于训练份额。进一步把语料配比 $p$ 提升为
第三层内生决策变量：配比联合优化显示最优配比向最高质量域（小说，
$Q=0.631$）集中，但配比杠杆仅约 2\%，直接质量投资才是主导途径；上下文
长度 $L_{\text{ctx}}$ 按题意保持\textbf{外生}（由架构与任务需求给定，不参与
内点寻优），在其 C7 可行域上的权衡分析（数据拟合弹性 0.197）表明：纯损失
口径取可行下限、实证收益口径取架构上限，是``能力换损失''的外生取值决策。跨问题\textbf{闭环外部
验证}：以 57 个真实开源训练点对照最优轨迹 $N^{*}(C)$（对数规模相关 0.92、
中位偏差 0.36 dex，72\% 落在 $\pm0.5$ dex 内），贴合度优于经典 Chinchilla
规则约 30\%，89.5\% 真实模型位于``数据相对充裕''一侧——算力—规模模型链
对真实训练实践自洽。

\textbf{问题四：技术演进分解与前沿预测。}以 C1 开源模型前沿为对象建立 90\%
分位数回归（LP/statsmodels/sklearn/L-BFGS-B/Adam 五框架交叉复算）
$\ln S=c+b_N\ln N+b_T t$（$b_N=0.364$、$b_T=0.089$），
把前沿能力
...
不超过 0.008，且最优交互形式在 B8 结构对照中同样成立，6.3 节 $R^2=0.984$）；结构性
转移阈值对三种成本函数均落在 $10^{18}$ 量级；前沿分解对分位数
（0.5--0.99）稳健：规模贡献占比随分位数单调上升（64.9\%$\to$88.8\%），
``规模主导''恒成立，且每分位五框架复算一致（$b_N$ 极差 $<$0.2\%、
$b_T$ 极差 $<$2\%）。
全套结论经受严格检验：参数与弹性 Bootstrap 区间（300 次重抽）、
175 个参考点上的弹性排序、家族留出与跨框架求解器收敛（24 初值、
九类框架一致）、抽样规模收敛性等均在正文给出统计依据，且四个问题的
拟合/优化/预测均经 scipy/statsmodels/sklearn/torch 独立实现交叉复算
一致（如配比岭回归五框架 $\kappa=0.314031$ 极差 $3\times10^{-10}$），
保证每项决策结论可复现、可复核。对四种标度律形式的\textbf{留出交叉验证}表明：引入
质量项使留出误差相对经典形式下降约 53\%（经典 RMSE 0.126$\to$加性 0.060），交互($N$)
形式留出最优（RMSE 0.051，相对经典降约 60\%），所选形式并非样本内过拟合；
\textbf{联合优化}相对纯规模基线在 $0.8\%$--$6.7\%$ 区间持续占优
