...
按照``模型建立 $\to$ 模型求解 $\to$ 模型验证''的流程组织本节。问题一要求
完成三项任务：（1）建立数据质量评分方法，给出样本级与域级质量分；（2）定义
并消解质量信号冲突；（3）建立 17 领域配比与损失之间的数量关系模型，并讨论
配比效应随规模的转移。A1--A3 提供约 27.3 万条样本、22 个质量指标（14 项
标量、8 项列表型），A4--A15 提供 1M/60M/1B 训练集与 10b/70b 估计五档规模
下的配比--损失数据，A16 给出 17 个配比域到 7 个质量域的映射。

\subsection{模型建立}

\subsubsection{数据质量评分模型}
...
为避免单一赋权法的偏差，采用熵权法\cite{refEntropy}与 CRITIC
法\cite{refCritic}按 $0.5:0.5$ 组合：
\begin{equation}
...
式 (\ref{eq:weights}) 表明熵权法强调信息量，CRITIC 强调指标强度与独立性，
二者按 $0.5:0.5$ 互补。

\paragraph{TOPSIS 综合评分}

在组合权重下构造加权标准化矩阵 $V_{ij}=w_j X_{ij}$，计算正理想解
$V^{+}$（各列最大值）与负理想解 $V^{-}$（各列最小值），样本质量分为
贴近度\cite{refTopsis}（式 \ref{eq:topsis}）：
\begin{equation}
...
\begin{sloppypar}%
\textbf{权重扰动鲁棒性}（图 \ref{fig:p1_weightrob}）：在官方全量口径
（$n=272{,}505$，域序与官方 \texttt{p1\_domain\_quality.csv} 的
$Q_{\text{weighted}}$ 域序 Spearman$=1.000$）上，对熵-CRITIC 组合权重施加
对数扰动 $w_j\leftarrow w_j e^{\sigma\varepsilon}$（$\varepsilon\sim
N(0,1)$，每档 200 次重抽）检验评分链对权重设定的敏感度：$\sigma=0.2$
（$\pm20\%$ 量级）时七域序与基线 Spearman 最小 0.964、均值 0.999，book
居首与 stackexchange 垫底概率均为 1.00；即使 $\sigma=0.5$（$\pm50\%$
量级），book 居首概率仍为 1.00、垫底概率 0.995——域序对权重扰动高度
稳健，``book 居首、低质域垫底''的结论不依赖单一权重标定。
\end{sloppypar}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{figures/v93_p1_weight_robust.png}
\caption{组合权重对数扰动（$\sigma=0.2$，200 次）的七域秩分布：book 恒为
第 1、stackexchange 恒为第 7。}
\label{fig:p1_weightrob}
\end{figure}
...
高度结构化的直觉一致。图 \ref{fig:p1_conflict} 是冲突指数的分布，可以发现
整体呈偏右拖尾形态、c4 域的冲突显著高于其他域，与上述统计完全吻合。
...
\label{eq:ridge}
\end{equation}
式 (\ref{eq:ridge}) 的岭回归\cite{refRidge}在配比高度共线（领域间相关性
高）时仍能给出
稳定系数。训练/测试按
...
\label{eq:shrink}
\end{equation}
拟合得到 $\kappa=0.314031$、$c=0.730$。图 \ref{fig:p1_shrink} 是配比系数
范数随模型规模的对数回归，可以发现三个尺度上的范数点严格落在幂律直线上，
收缩趋势清晰。这一``跨尺度收缩''现象表明：配比
...
\caption{配比系数范数随模型规模幂律收缩（$\kappa=0.314031$）。}
\label{fig:p1_shrink}
\end{figure}
...
\textbf{配比-损失系数与 $\kappa=0.314031$ 的收缩结论对估计器实现数值稳健}。
...
的阶梯，与跨尺度系数收缩（$\kappa=0.314031$）形成\textbf{独立互证}：收缩规律
...
作为第三层杠杆的建模选择。对多样性上限 $c$（单域配比上限）做连续扫描
（$c=10\%$--$100\%$，v98）进一步确认：处方预测降幅单调饱和地从
$4.9\%$（$c=10\%$）升至 $16.9\%$（$c=100\%$，无约束上界），而实测
最优混合行收益始终约 $3\%$ 不随 $c$ 变化——``有限杠杆''结论在单域
上限全域稳健（收益量级不变，仅处方形态随 $c$ 增大而逐渐集中）。
...
份额越高、该域验证损失越低，配比模型与``同域强化''的直觉一致
（跨域迁移结构与文献中的迁移标度律\cite{refHernandez}方向相符）；
...
图 \ref{fig:p1_domain_q} 展示了七域域级质量分的排序对照，可以发现加权口径
与 TOPSIS 对照口径的排序完全一致。书域（Gutenberg 等长文、排版规范）
质量最高，arxiv 次之；github 与
...
因 $n=81$k 而极度显著）。图 \ref{fig:p1_domainicc} 展示了域内质量分的
离散程度，可以发现各域内标准差远大于域间差异、域内文档质量高度分化。
这说明：域间差异\textbf{统计显著、排序稳定}
...
问题一的验证由三类手段构成：\textbf{可视化验证}（图
\ref{fig:p1_methods}--\ref{fig:p1_domdist} 等 20 余幅给出域序、分布与筛选
曲线的直接图证）、\textbf{数值交叉验证}（八方法域序 Kendall $W=0.69$ 与
加权/折衷族 $W=0.98$ 置换检验 $p<0.001$；五框架岭回归 $\kappa=0.314031$
极差 $3\times10^{-10}$；抽样 5\%--100\% 收敛）与\textbf{物理/几何规律对照}
（跨尺度收缩预测大模型配比效应趋零，v35 的 1B 外推失败正是该预测的直接
表现；book 画像独特性与质量分居首互相印证）。三项验证均通过，证实了本问
模型的合理性与配比效应结论的稳健性。

问题一建立了``预处理 $\to$ 组合赋权 $\to$ TOPSIS''的质量评分链，定义了
双族冲突指数并给出对称截尾消解规则，得到 7 域质量分；进一步用岭回归刻画
17 域配比与 13 损失域的数量关系，并通过跨尺度系数收缩（$\kappa=0.314031$）
