...
域级质量排序在消解前后保持\textbf{簇级稳定}（book/c4 前二簇、低质域垫底，
详见下文 v89 核验），验证了方法的稳健性。
...
截尾消解}本身（v89，官方聚合口径）：$k\in\{2,4,6,8\}$ 时消解后与加权的
域序 Spearman 为 0.71--0.93，book/c4 构成前二簇（分数差仅约 0.004）、
低质域垫底不变——域序在\textbf{簇级保持}而非逐位不变，消解的作用是校正
高冲突样本的分数而非改写域序。以上与 v28 的 $k$ 敏感性（0.68--0.96 对齐）
一起构成对质量评分管道两个自由度（权重、冲突消解）的完整稳健性检验。
