...
\begin{table}[htbp]
\centering
\caption{广义标度律拟合（B6+B7）}
\begin{tabular}{c|ccccccccc}
\toprule
形式 & $E$ & $A$ & $a$ & $B$ & $b$ & $C$ & $g$ & $d$ & $R^2$ \\
\midrule
加性 & 1.551 & 0.534 & 0.281 & 1.251 & 0.310 & 0.445 & 0.991 & 0.9716 \\
交互 & 1.551 & 0.534 & 0.281 & 1.251 & 0.310 & 0.445 & 0.991 & 0.0447 & 0.9720 \\
\bottomrule
\end{tabular}
\end{table}

两形式 $R^2$ 仅差 $4\times10^{-4}$，说明 $Q$ 项结构稳定；交互项指数
$d=0.045$ 很小，即质量损失随 $D$ 的衰减在拟合范围内并不显著。取交互形式
为最终模型（$Q=1$ 时自动退化为经典标度律，满足可退化性要求）。广义模型
参数与经典拟合（$E\approx1.69$）相比 $E$ 略降为 1.551，是因为质量项
$(1-Q)^{g}$ 解释了原经典模型中归入不可约误差的部分方差。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/p2_generalized_fit.png}
\caption{广义标度律在 B6+B7 数据上的拟合（交互形式，$R^2=0.972$）。}
...
\begin{equation}
\varepsilon_N=-0.062,\qquad \varepsilon_D=-0.030,\qquad
\varepsilon_Q=-0.141.
\label{eq:elasticity}
\end{equation}
质量弹性显著大于参数量与数据量弹性（绝对值约为参数量的 2.3 倍、数据量的
4.7 倍），直观表明：在基准点附近，改善数据质量是比增加参数/数据更高效的
...
\label{eq:equiv}
\end{equation}
在基准点处质量每提升 0.1 等价于增加 $\Delta N=0.229$B 参数（约 23\% 相对
增量），进一步说明质量杠杆的高效性。

\subsection{小结}

问题二完成了从经典标度律到广义标度律的推广：经典参数与文献一致
（$\alpha=0.34,\beta=0.28$），跨家族偏移修正验证 $R^2=0.83$；广义形式
$L=E+A N^{-a}+B D^{-b}+C(1-Q)^{g}D^{-d}$ 在 B6+B7 上 $R^2=0.972$，
$Q=1$ 可退化；弹性分析表明质量弹性最高，质量 0.1 等价于 0.229B 参数；
