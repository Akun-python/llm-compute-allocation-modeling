...
\label{eq:frontier}
\end{equation}
其中 $t$ 为以 2022 年为 0 的年份。分位数回归（迭代重加权最小二乘实现）对
前沿点稳健，比均值回归更能刻画``能力前沿''。拟合结果（$n=2493$）：
\begin{equation}
c=2.575,\qquad b_N=0.319,\qquad b_T=0.091.
\label{eq:frontier_fit}
\end{equation}
解释：$b_N=0.319$ 表示参数每翻倍、前沿平均分约提升 $\ln2\times0.319
\approx 22\%$（对数尺度 0.22）；$b_T=0.091$ 表示即使参数不变，每年前沿
...
3.5 倍）。代入得：
\begin{equation}
\text{规模贡献}=b_N g_N=0.400\ (81.4\%),\qquad
\text{非规模技术贡献}=b_T=0.091\ (18.6\%).
\label{eq:decomp}
\end{equation}
结论：过去数年开源模型前沿能力的提升约 81\% 来自规模扩张（更大参数量），
约 19\% 来自非规模技术进步。这一分解表明，即便算力增长放缓（参数量增速
减半），非规模技术进步仍能维持约 0.091/年的底线增速——这是预测情景构建的
关键。
...
\begin{itemize}
\item \textbf{基准点}：2025 年分位数前沿水平 $\hat S_{2025}=40.8$（对应
...
时间 & 预测中位 & 90\% 区间 & & 90\% 区间 \\
\midrule
2026.0（12 个月） & 66.7 & [57.2, 77.6] & 54.6 & [46.3, 63.8] \\
2027.0（24 个月） & 109.2 & [92.5, 125.3] & 73.2 & [63.1, 86.0] \\
\bottomrule
\end{tabular}
\end{table}

基础情景下 12 个月前沿平均分约从 40.8 升至 66.7（+63\%）、24 个月升至
109.2（+168\%）；放缓情景下分别为 54.6（+34\%）与 73.2（+79\%）。放缓
情景与基础情景的差距随预测期扩大：24 个月时放缓情景比基础情景低约 33 分，
...
问题四建立了分位数前沿回归 $\ln S=2.575+0.319\ln N+0.091t$，把前沿能力
年增速分解为规模 81.4\% 与非规模 18.6\%；基于 C4 参数增速构建基础与放缓
情景，预测 12/24 个月前沿（基础 66.7/109.2，放缓 54.6/73.2，含 90\% 区间）；
