...
（2019--2025），C4 为 Epoch 模型库\cite{refEpoch}（含算力、参数、开源权重标记），C5/C6
...
复算（v80）：主链路全开源子集（$n=2493$）$b_N=0.364315$、$b_T=0.089045$、规模
...
（$n=2493$）：
\begin{equation}
c=2.480669,\qquad b_N=0.364315,\qquad b_T=0.089045.
\label{eq:frontier_fit}
\end{equation}
解释：$b_N=0.364315$ 表示参数每翻倍、前沿平均分（对数尺度）提升
$\ln2\times0.364315\approx0.2525$，对应水平增幅约 $28.7\%$；$b_T=0.089045$
表示即使参数不变，每年前沿
...
梯度下降。结果：\textbf{五框架复现同一系数}——$c_0=2.480669$、$b_N=0.364315$、
$b_T=0.089045$（相对极差：$c_0$ 为 $4\times10^{-3}\%$、$b_N$ 为
...
\text{非规模技术贡献}=b_T=0.089045\ (16.3\%).
\label{eq:decomp}
\end{equation}
式 (\ref{eq:decomp}) 的结论：过去数年开源模型前沿能力的提升约 84\% 来自
规模扩张（更大参数量），
...
图 \ref{fig:p4_decomp} 展示了前沿能力增长的分解结果，可以发现规模扩张
贡献（83.7\%）远大于非规模技术贡献（16.3\%），两个扇区的占比与前文
$b_N$、$b_T$ 系数核算完全一致。
...
Benchmark 平均分建立 logit--log 线性映射（式 \ref{eq:bridge}）：
\begin{equation}
...
场景可用，且高可比性样本误差更集中。图 \ref{fig:p4_bridge} 展示了桥接
映射的散点与拟合直线，可以发现中低精度样本紧贴 logit 直线、高可比性样本
残差更小。该桥接可将问题二的损失预测换算为能力
...
问题四建立了分位数前沿回归 $\ln S=2.480669+0.364315\ln N+0.089045t$
，把前沿能力
