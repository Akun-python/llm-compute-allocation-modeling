...
$g(0.4)$：指数 $1.1\times10^{8}$、幂 $1.28\times10^{8}$、对数 $3.22\times
10^{9}$（对数型在基线处成本已较高，故其质量激活阈值略大，见敏感性分析）。

质量通道的两阶段行为由成本函数的两个特征量决定（第 \ref{sec:p3_transition} 节全程轨迹分析）：
...
$3.2\times10^{18}$ 即饱和，指数型 $1.8\times10^{20}$、幂型 $8.9\times10^{19}$）。
...
\begin{itemize}
\item A1--A3 质量指标原始记录含 27 个字段（其中 22 项质量指标：14 项标量
+ 8 项列表型，另含 id/域标签等辅助字段）；列表型指标取均值压缩为标量后进入
评分（22 列），共 272505 条有效样本（A1 51230 + A2 17523 + A3 203752；
域级评分统计覆盖其中 272486 条，19 条缺域标签计入总量）；负向指标方向
