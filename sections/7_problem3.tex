...
\midrule
$10^{19}$ & 指数 & 0.319 & 3.56 & 0.717 & 3.252 & 0.683 & 0.224 & 0.093 \\
$10^{19}$ & 幂 & 0.296 & 4.05 & 0.584 & 3.290 & 0.719 & 0.183 & 0.098 \\
$10^{19}$ & 对数 & 0.413 & 2.28 & 1.000 & 3.206 & 0.564 & 0.359 & 0.077 \\
\midrule
$10^{22}$ & 指数 & 9.83 & 140.9 & 1.000 & 2.102 & 0.831 & 0.055 & 0.113 \\
$10^{22}$ & 幂 & 9.97 & 137.2 & 1.000 & 2.103 & 0.821 & 0.067 & 0.112 \\
$10^{22}$ & 对数 & 9.46 & 151.3 & 1.000 & 2.099 & 0.859 & 0.024 & 0.117 \\
\midrule
$10^{24}$ & 指数 & 103.6 & 1407.9 & 1.000 & 1.828 & 0.875 & 0.006 & 0.119 \\
$10^{24}$ & 幂 & 103.7 & 1403.8 & 1.000 & 1.828 & 0.874 & 0.007 & 0.119 \\
$10^{24}$ & 对数 & 103.2 & 1418.0 & 1.000 & 1.828 & 0.878 & 0.002 & 0.120 \\
\bottomrule
\end{tabular}
\end{table}
...
有效杠杆（$10^{19}$ 量级质量份额 18\%--36\%），到 $10^{22}$ 后 $Q^{*}$ 饱和
于 1，质量份额迅速萎缩（降至 2\%--7\%），新增预算全部转向规模。
...
成本函数 & 指数型 & 幂函数型 & 对数渐进型 \\
\midrule
$Q$ 激活阈值 $C^{*}$ & $1.12\times10^{18}$ & $3.16\times10^{18}$ & $3.55\times10^{18}$ \\
\bottomrule
\end{tabular}
\end{table}
...
$\sim C(1-Q_0)^{g-1}D^{-d}$，后者 $\sim g'(Q_0)D$，两者交叉处即
$C^{*}$。三档预算中 $10^{19}$ 高于阈值、$10^{22}/10^{24}$ 远高于阈值，
故都呈现质量投入（前已述 $10^{22}$ 起 $Q^{*}=1$ 饱和）。成本主导成分在三档
预算下始终为训练，未发生切换，说明本问题中质量与注意力始终未越过训练成为
主导成本——结构性转移主要表现为\textbf{质量变量的边界--内点切换}。
...
\midrule
2048 & 0.304 & 4.20 & 0.578 & 0.766 & 0.052 \\
4096 & 0.296 & 4.05 & 0.584 & 0.719 & 0.098 \\
8192 & 0.280 & 3.80 & 0.593 & 0.639 & 0.174 \\
\textbf{30000} & \textbf{0.227} & \textbf{2.95} & \textbf{0.632} & \textbf{0.401} & \textbf{0.401} \\
32768 & 0.222 & 2.88 & 0.636 & 0.383 & 0.418 \\
131072 & 0.140 & 1.74 & 0.722 & 0.146 & 0.639 \\
\bottomrule
\end{tabular}
\end{table}

$L_{\text{ctx}}$ 越长，注意力份额越高、最优规模越小：$L_{\text{ctx}}$ 从
2048 增至 131072，$N^{*}$ 从 0.304B 降至 0.140B，$D^{*}$ 从 4.20B 降至
1.74B，损失从 3.25 升至 3.37（未列出）。在临界点 $L_{\text{ctx}}^{\text
{crit}}=30000$ 处注意力份额恰好等于训练份额（0.401 vs 0.401），与解析
