...
全部代码位于 \texttt{solve/code/}，由 \texttt{common.py}（路径与常量）、
\texttt{p1\_quality.py}（问题一评分与冲突消解）、\texttt{p1\_mixture.py}
（问题一配比建模）、\texttt{p2\_scaling.py}（问题二标度律）、
\texttt{p3\_optimization.py}（问题三优化）、\texttt{p4\_evolution.py}
（问题四分解预测）六个脚本构成，运行环境为 Python 3.14 + numpy 2.4 +
pandas 3.0 + scipy 1.17 + scikit-learn 1.8 + matplotlib 3.10，结果写入
\texttt{solve/results/}，插图写入 \texttt{solve/figures/}。以下列出每个
脚本的核心代码片段（完整代码随提交文件一并提供）。
...
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\small]
def fit_generalized(N, D, Q, L, form="interaction"):
    """最小二乘拟合 L = E + A N^-a + B D^-b + C(1-Q)^g D^-d"""
    def resid(p):
        E, A, a, B, b, C, g = p[:7]
        d = p[7] if form == "interaction" else 0.0
